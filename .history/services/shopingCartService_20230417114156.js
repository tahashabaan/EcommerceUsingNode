...
exports.applyCoupon = asyncHandler(async (req, res, next) =>{
  ...
  const coupon = await Cupon.findOne({name:req.body.name,expireDate:{$gt:Date.now()}});
  ...
  const totalPriceC = cart.totalPrice; // 400 10
  cart.totalPriceAfterDisCount = totalPrice - (totalPrice * (coupon.discount/100))
 
  await cart.save();
 
  // if(!cupon)
  //    return next(new ApiError('code not found'))
