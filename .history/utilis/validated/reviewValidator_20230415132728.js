...
exports.createReviewValidator = [
    ...
    .custom((val, {req}) => {
        Rview.findOne({user:req.user._id, product:req.body.product}).then((review) => {
            if (review) {
                return  Promise.reject(new Error('the user add review already'))
            }
        })
        }),
    check('user')
...
exports.updateReviewValidator = [
    ...
    .custom(async (idVal, {req}) => {
       ...
       if (!reviewUser) 
          return new Promise.reject(new Error('not can find User and Not can update review'))
       //check  the smama user create review
          if(reviewUser.user._id.toString() !== req.user._id.toString()) 
             return new Promise.reject(new Error('not allowed perform this product'))
        })
